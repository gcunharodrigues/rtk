...
- **[`codex/`](codex/README.md)** — Native `PreToolUse` hook (`Bash` → `rtk hook codex`) in global `hooks.json`, plus optional awareness
...
| Codex CLI | Rust binary (`rtk hook codex` via `hooks.json`) | Transparent rewrite | Yes (`updatedInput`) |
