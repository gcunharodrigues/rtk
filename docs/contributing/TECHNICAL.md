...
| [`codex/`](../hooks/codex/README.md) | OpenAI Codex CLI | Global `rtk init -g --codex` installs `PreToolUse`; `--hook-only` skips awareness; local mode is awareness-only |
...
| Codex CLI | Rust binary | Global `PreToolUse`: `Bash` → `rtk hook codex`; local `AGENTS.md` is awareness-only | Yes (`updatedInput`; Codex owns approval) |
